...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            .dashboard-shell {
                background-image: radial-gradient(circle at top left, rgba(59, 130, 246, 0.18), transparent 45%),
    ...
    <body class="bg-slate-100 text-slate-900">
        <div class="min-h-screen dashboard-shell">
            <x-dashboard.navbar role="Siswa" />

            <div class="flex">
                @php
                    $menu = [
                        ['label' => 'Dashboard', 'url' => url('siswa/dashboard'), 'active' => request()->is('siswa/dashboard')],
                        ['label' => 'Profil', 'url' => url('siswa/profile'), 'active' => request()->is('siswa/profile')],
                        ['label' => 'Nilai', 'url' => url('siswa/nilai'), 'active' => request()->is('siswa/nilai')],
                        ['label' => 'Materi', 'url' => url('siswa/materi'), 'active' => request()->is('siswa/materi')],
                        ['label' => 'Jadwal', 'url' => url('siswa/jadwal'), 'active' => request()->is('siswa/jadwal')],
                        ['label' => 'Pengumuman', 'url' => url('siswa/pengumuman'), 'active' => request()->is('siswa/pengumuman')],
                    ];
                @endphp
                <x-dashboard.sidebar :items="$menu" role="Siswa" />

                <main class="dashboard-main flex-1 px-6 py-8">
                    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Ruang Siswa</p>
                        ...
                        <div class="mb-6 lg:hidden">
                            <div class="flex gap-2 overflow-x-auto pb-2">
                                @foreach($menu as $item)
                                    ...
                                    @endphp
                                    <a href="{{ $item['url'] }}" class="rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] {{ $active ? 'border-blue-500 text-blue-600' : 'border-slate-200 text-slate-500' }}">
                                        {{ $item['label'] }}
                                    </a>
                                @endforeach
                            </div>
